...
# Read the setting to determine if the line chart should be generated
GENERATE: bool = config.getboolean("Settings", "generate_lines_of_code_line_chart")
...
fig = px.line(
    ...
    line_shape="linear",  # Use a linear line shape
    text="Lines of Python Code",
    labels={"Lines of Python Code": "Lines of Code"},  # Set y-axis label
    template="plotly_dark",  # Use dark theme template
)

# Update the trace to display the value of lines of code on top of the line
fig.update_traces(
    texttemplate="%{y}",
...
fig.update_layout(
    ...
    xaxis=dict(
        showgrid=False,
        showticklabels=True,  # Display tick labels on the x-axis
        tickangle=-45,  # Rotate x-axis labels for better readability
    ),
    yaxis=dict(
        showgrid=False,
        showticklabels=True,  # Display tick labels on the y-axis
        zeroline=False,
        visible=True,
        showline=True,
...
# Check if the line chart should be generated and saved as an image
if GENERATE:
